...
namespace cp {
    ...
    struct comp {
        ...
        sf::String                          input;
        std::array<2, sf::RectangleShape>   box_;
        std::array<3, sf::Text>             text_;
        std::array<2, sf::Font>             font;
        ...
        /// initialize elements
        /// if error
        ///     \# sets emergency_terminate or gbl::end (if already prints error)
        ///     \# return early
        void init () {
            if (!font[0].loadFromFile(fp::framd_font) || !font[1].loadFromFile(fp::consola_font)) {
                gbl::end = true;
                return;
            }
